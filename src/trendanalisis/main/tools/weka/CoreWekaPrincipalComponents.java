...
import trendanalisis.main.method.VarimaxForPCA;
 ...
 */
public class CoreWekaPrincipalComponents extends UnsupervisedAttributeEvaluator
    ...
    /*
     * add neew varible default 0
     * rizky  munggaaran
     */
    private double m_thresholdSumEigen = 0.0;
    private boolean bol_varimax = false;
    private double[][] dataVarimax;
    private boolean kaiserMayer = false;
    private VarimaxForPCA varimax = null;
    private double thresholdSumEigen = 0;
    private double thresholdcoverVarince = 0;
    private double num_kaiserMayer = 1;
    ...
    /**
     * Gets the current settings of CoreWekaPrincipalComponents
     * 
    ...
    private void buildAttributeConstructor(Instances data) throws Exception {
        ...
        if (m_thresholdSumEigen == 0) {
            m_thresholdSumEigen = m_sumOfEigenValues;
    ...
    public Instances transformedData(Instances data) throws Exception {
        ...
        for (int i = 0; i < data.numInstances(); i++) {
            ...
            if (bol_varimax) {
                ...
                converted = convertInstanceVarimax(data.instance(i));

            } else {
                converted = convertInstance(data.instance(i));
            }
    ...
    private String principalComponentsSummary() {
        ...
        if (bol_varimax) {
            result.append("VARIMAX \n");
            for (int j = 0; j < dataVarimax.length; j++) {

                for (int i = 0; i < dataVarimax[j].length; i++) {
                    result.append(Utils.doubleToString(
                            dataVarimax[j][i], 7, 4) + "\t");
                }
                result.append(m_trainInstances.attribute(j).name() + '\n');
        ...
        for (int i = m_numAttribs - 1; i >= 0; i--) {
            ...
            cumulative += m_eigenvalues[m_sortedEigens[i]];
             ...
             if ((cumulative / m_sumOfEigenValues) >= m_coverVariance) {
                    thresholdSumEigen = cumulative;
                    thresholdcoverVarince = cumulative / m_sumOfEigenValues;
                    break;
                }
            if (kaiserMayer) {

                if (m_eigenvalues[m_sortedEigens[i-1]] < num_kaiserMayer) {
                ...
                /// if (hit >= 7) {
                //    System.out.println("-------  " + "  NONE  " + cumulative);
                if ((cumulative) >= m_thresholdSumEigen) {
                    thresholdSumEigen = cumulative;
                    thresholdcoverVarince = cumulative / m_sumOfEigenValues;
    ...
    public Instance convertInstanceVarimax(Instance instance) throws Exception {
        ...
        newVals = new double[numAttributSelected];

        double m_eigenvectorsSort[][] = new double[m_numAttribs][numAttributSelected];

        for (int j = 0; j < m_numAttribs; j++) {
            hit = 0;
            for (int i = m_numAttribs - 1; i >= m_numAttribs - numAttributSelected; i--) {

                /*
                 * faktor loading
                 * Pij=Eij*sqrt(eigen vektor j)
                 */
                m_eigenvectorsSort[j][hit] =    m_eigenvectors[j][m_sortedEigens[i]] * Math.sqrt(m_eigenvalues[m_sortedEigens[i]]);
                hit++;
            }

        }
        RealMatrix m = MatrixUtils.createRealMatrix(m_eigenvectorsSort);
        m_eigenvectorsSort = m.transpose().getData();

        varimax = new VarimaxForPCA(VarimaxForPCA.RotationMethod.VARIMAX, null, 1);
        varimax.setData(m_eigenvectorsSort);
        varimax.setKaiserNormalisation(true);
        varimax.rotate();

        dataVarimax = MatrixUtils.createRealMatrix(varimax.getData()).transpose().getData();


        cumulative = 0;
        hit = 0;
        for (int i = 0; i < numAttributSelected; i++) {
            double tempval = 0.0;
            for (int j = 0; j < m_numAttribs; j++) {


                tempval += (dataVarimax[j][i] * tempInst.value(j));
                // tempval += (m_eigenvectors[j][m_sortedEigens[i]] * tempInst.value(j));
            }
            newVals[numAttributSelected - i - 1] = tempval;
    ...
    public String getRevision() {
        return RevisionUtils.extract("$Revision: 10493 $");
    }
    ...
    public static void main(String[] argv) {
        runEvaluator(new CoreWekaPrincipalComponents(), argv);
    }

    public double getM_SumOfEigen() {

        return m_sumOfEigenValues;
    }

    /**
     * @return the m_thresholdSumEigen
     */
    public double getM_thresholdSumEigen() {
        return m_thresholdSumEigen;
    }

    /**
     * @param m_thresholdSumEigen the m_thresholdSumEigen to set
     */
    public void setM_thresholdSumEigen(double m_thresholdSumEigen) {
        this.m_thresholdSumEigen = m_thresholdSumEigen;
    }

    /**
     * @return the bol_varimax
     */
    public boolean isBol_varimax() {
        return bol_varimax;
    }

    /**
     * @param bol_varimax the bol_varimax to set
     */
    public void setBol_varimax(boolean bol_varimax) {
        this.bol_varimax = bol_varimax;
    }

    public void setKaiserMayer(boolean bol_kaiserMayer) {
        this.kaiserMayer = bol_kaiserMayer;
    }

    public boolean isKaiserMayer() {
        return this.kaiserMayer;
    }

    /**
     * @return the varimax
     */
    public VarimaxForPCA getVarimax() {
        return varimax;
    }

    /**
     * @param varimax the varimax to set
     */
    public void setVarimax(VarimaxForPCA varimax) {
        this.varimax = varimax;
    }

    /**
     * @return the thresholdcoverVarince
     */
    public double getThresholdcoverVarince() {
        return thresholdcoverVarince;
    }

    /**
     * @param thresholdcoverVarince the thresholdcoverVarince to set
     */
    public void setThresholdcoverVarince(double thresholdcoverVarince) {
        this.thresholdcoverVarince = thresholdcoverVarince;
    }

    /**
     * @return the num_kaiserMayer
     */
    public double getNum_kaiserMayer() {
        return num_kaiserMayer;
    }

    /**
     * @param num_kaiserMayer the num_kaiserMayer to set
     */
    public void setNum_kaiserMayer(double num_kaiserMayer) {
        this.num_kaiserMayer = num_kaiserMayer;
    }
}
