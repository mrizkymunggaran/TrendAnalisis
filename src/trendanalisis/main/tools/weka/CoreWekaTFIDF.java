 ...
 */
public class CoreWekaTFIDF extends Filter implements UnsupervisedFilter,
        OptionHandler {
     ...
     */
    private int m_WordsToKeep = 100000;
    /**
    ...
    /** normalization: Normalize costume. */
    public static final int FILTER_NORMALIZE_COSTUMIZE = 3;
    /**
    ...
    public static final Tag[] TAGS_FILTER = {
        ...
        new Tag(FILTER_NORMALIZE_COSTUMIZE, "Normalize all data costume")};
    /**
    ...
    private double[] NormalLengt;
    private boolean binary_transform = false;
    private double[] IG;
    private double val_Entropy = 0;
    private boolean stat_pos_word = false;
    ...
    public boolean isStat_pos_word() {
        return stat_pos_word;
    }

    public void setStat_pos_word(boolean stat_pos_word) {
        this.stat_pos_word = stat_pos_word;
    }

    /**
     * Default constructor. Targets 1000 words in the output.
     */
    public CoreWekaTFIDF() {
    }
     ...
     *          assigned).
     */
    public CoreWekaTFIDF(int wordsToKeep) {
        m_WordsToKeep = wordsToKeep;
    }

    /**
     * @return the binary_transform
     */
    public boolean isBinary_transform() {
        return binary_transform;
    }

    /**
     * @param binary_transform the binary_transform to set
     */
    public void setBinary_transform(boolean binary_transform) {
        this.binary_transform = binary_transform;
    }
    ...
    public boolean input(Instance instance) throws Exception {
        ...
        if (isFirstBatchDone()) {
            ...
            int firstCopy = convertInstancewoDocNorm(instance, fv, 0);
            Instance inst = fv.get(0);
            if (m_filterType != FILTER_NONE) {
                normalizeInstance(inst, firstCopy, 0);
            }
            push(inst);
            return true;
        } else {
    ...
    public boolean batchFinished() throws Exception {
        ...
        if (!isFirstBatchDone()) {
            ...
            for (int i = 0; i < m_NumInstances; i++) {
                firstCopy = convertInstancewoDocNorm(getInputFormat().instance(i), fv, i);
            }

            val_Entropy = FeatureSelection.Entropy(global_tf);

            // Need to compute average document length if necessary
            if (m_filterType != FILTER_NONE || m_filterType != FILTER_NORMALIZE_COSTUMIZE) {
                m_AvgDocLength = 0;
            ...
            NormalLengt = new double[m_NumInstances];
            // Perform normalization if necessary.
            if (m_filterType == FILTER_NORMALIZE_ALL || m_filterType == FILTER_NORMALIZE_COSTUMIZE) {
                for (int i = 0; i < fv.size(); i++) {
                    normalizeInstance(fv.get(i), firstCopy, i);
                }
            }
    ...
    private int convertInstancewoDocNorm(Instance instance, ArrayList<Instance> v, int indexInstance) {
        ...
        if (m_IDFTransform == true) {
            ...
            for (; it.hasNext();) {
                ...
                if (index.intValue() >= firstCopy) {
                    double val = contained.get(index).doubleValue();
                    double valIG = (FeatureSelection.InformationGain(m_DocsCounts[index.intValue()], m_NumInstances));// Math.abs(valIG);

                    global_tf[index.intValue()] += val;
                    df_prob[index.intValue()] = FeatureSelection.IdfProbability((double) m_NumInstances, m_DocsCounts[index.intValue()]);
                    ...
                    val = val
                            * (1 + Math.log((double) m_NumInstances / (double) m_DocsCounts[index.intValue()]));
    ...
    private double ScoreTittle(ArrayList<String> subtittles, int index) {
        double score = 0;
        if (isFitur_subtitle()) {
            String content = Words.get(index);

            if (subtittles.indexOf(content) != -1) {
                score = (0.5 * Math.log(1 + global_tf[index])) + (0.5 * Math.log(1 + m_DocsCounts[index]));
    ...
    public static void main(String[] argv) {
        runFilter(new CoreWekaTFIDF(), argv);
    }

    public int[] GetNumDocAttribut() {
        return m_DocsCounts;
    }

    public double[] GetLabelAttribut() {
        return global_tf;
    }
