...
//{
    ...
    public Vector<Vector<String>> largeitemset = new Vector<Vector<String>>();
    public Map<String, Integer> Haslargeitemset = new HashMap<String, Integer>();
    ...
    public String transafile = "data/apriori_label/label.txt"; // default transaction file
        ...
        if (htnf.nodeattr == IL) {
            ...
            for (j = 1; j <= tempvec.size(); j++) {
              ...
              //  double conf=1;
                ...
                int sup = ((itemsetnode) tempvec.elementAt(j - 1)).counter;
              //  if (sup >= minsup) {
                if (sup >= ((minsup * M) / 100)) {
                    
                     lis.addElement(((itemsetnode) tempvec.elementAt(j - 1)).itemset);
                       
                     Haslargeitemset.put(((itemsetnode) tempvec.elementAt(j - 1)).itemset, ((itemsetnode) tempvec.elementAt(j - 1)).counter);
                }
            }
        } else {
            if (htnf.ht == null) {
...
//  Return     :
//-------------------------------------------------------------
    public Apriori(int minsup, Instances ins, String PATH) throws IOException {
...
//        System.out.println();
        ...
        // getconfig();
        this.minsup = minsup;
        this.N = ins.numAttributes();
        this.M = ins.numInstances();
...
//            System.out.println((Vector) (largeitemset.elementAt(k - 1System.out.println)));
        }
